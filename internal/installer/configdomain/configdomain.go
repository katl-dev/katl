...
func hostConfigurationFiles(config manifest.HostConfiguration) ([]confext.NativeEtcFile, error) {
	...
	if len(config.Sysfs) > 0 {
		...
		content.WriteString("# Generated by Katl from hostConfiguration.sysfs.\n")
		for _, setting := range settings {
			value := base64.StdEncoding.EncodeToString([]byte(setting.Value))
			fmt.Fprintf(&content, "w~ %s - - - - %s\n", setting.Name, value)
		}
		files = append(files, confext.NativeEtcFile{
			Path:    manifest.HostConfigurationSysfsTmpfilesPath,
