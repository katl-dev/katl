...
func mergeSourceHostConfiguration(base, next SourceHostConfiguration) SourceHostConfiguration {
	...
	if units, ok := next.EnabledUnits.Get(); ok {
		out.EnabledUnits = supplied(slices.Clone(units))
	}
...
func lowerHostConfiguration(config SourceHostConfiguration) manifest.HostConfiguration {
	...
	return manifest.HostConfiguration{EnabledUnits: slices.Clone(config.EnabledUnits.Value()), MaskedUnits: slices.Clone(config.MaskedUnits.Value()), Sysfs: sysfs, Sets: sets}
}
...
func cloneSourceHostConfiguration(config SourceHostConfiguration) SourceHostConfiguration {
	out := config
	out.EnabledUnits = cloneOptionalSlice(config.EnabledUnits)
