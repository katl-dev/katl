...
func TestMetadataWriters(t *testing.T) {
	...
	if got := strings.Join(rootMetadata.CompatibleBoot.KernelCommandLine, " "); !strings.Contains(got, "console=ttyS0,115200n8 console=tty3 systemd.getty_auto=no") {
	...
	if got := strings.Join(ukiMetadata.KernelCommandLine, " "); !strings.Contains(got, "console=ttyS0,115200n8 console=tty3 systemd.getty_auto=no") {
