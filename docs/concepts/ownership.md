...
| Katl node-management trust | Operator | Trusted-network access is the default; optional mTLS uses project-held secrets retained across reinstalls. Workstation context is disposable. |
...
TCP `9443` defaults to trusted-network access for new configurations, with
optional mTLS for authentication and encryption. SSH is key-only when
