...
| `/var/lib/katl/ssh/host-keys` | sshd, `katl-ssh-host-keys` | Stable SSH host identity | Native `/var`; validated before sshd starts, with missing or invalid keys replaced atomically |
| `/etc/ssh/ssh_host_*` | distribution sshd packaging | Disabled distribution host-key paths | Katl configures sshd to read its persistent key directly from `/var/lib/katl/ssh/host-keys`; generated confext may own sshd config snippets but not host keys |
